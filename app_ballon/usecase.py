...
def baloon_calculate(dto_input) -> str:
    ...
    # todo: через ssh соединение перезагрузить сервер и посмотреть поднимится ли проект
    # todo: почитать про cicd
    # todo: настроить гитигноре для проекта и занести env
    #todo: в прод.докер убрать режим дебага перекинуть в env
    #todo: сделать нагрузочное тестирование через wrk или через любую консольную утилитку(и определить rps)
    #todo: удалить дебаг из докер компосе
