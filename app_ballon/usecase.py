...
def baloon_calculate(dto_input) -> str:
    ...
    # todo: почтитать отличие ядер (не в нейросети) от процессора, процессов и от тредов.
    # todo: сделать в docker-compose.prod чтобы он автоматически поднимался при сбое.
    # todo: сделать обновление на сайте и протащить на сервер все изменения
    #todo: сделать лэндинг для проекта, под Django, он должен открывться при переходе на главную страницу
