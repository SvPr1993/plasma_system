...
def baloon_calculate(dto_input) -> str:
    ...
    # todo: реализовать в этом проекте троттлер в джанго и реализовать через кастомный middleware и включать тротлер через переменное окружение и задать рпс через .env+
    # todo: переписать файл agents, прописать маршрут по алгоритму составлять и запускать проект для работы где-либо
    # todo: сделать celery для этого проекта, должно быть похоже на прокдакшн работу(+-)
    # todo: почитать про kafka
    # todo: изменить в dockerfile версию для python, чтобы он загружал целеком
    # todo: разобраться почему контейнеры сильно загружаются и поставить убунту хотя бы на флэшку или на sd диск, opencode попробывать или кодекс
